<template>
    <div
            ...
            <div class="max-h-[calc(100%-52px)] overflow-y-auto">
                <template v-for="(chat, id) in chatList" :key="id">
                    <div
                        ...
                        <p class="overflow-hidden text-ellipsis w-[calc(100%-30px)] pr-[6px] float-left cursor-pointer"
        ...
        <div class="float-left w-[calc(100%-161px)] h-full">
                ...
                <template v-if="currentChatId">
                    <template v-for="(chat, i) in chatList[currentChatId].chat" :key="i">
                        <div v-if="chat.type === 'model'" class="chat_content mb-[14px]">
                            <img src="../../assets/image/gemini.svg" class="float-left w-[40px] h-[40px] rounded-[6px]"
                                style="box-shadow: rgba(100, 100, 111, 0.2) 0px 7px 29px 0px;">
                            <div :id="`ai_chat_content_${i}`"
                        ...
                        <div v-if="chat.type === 'user'" class="chat_content mb-[14px]">
                            <img class="float-right w-[40px] h-[40px] rounded-[6px]"
                                style="box-shadow: rgba(0, 0, 0, 0.24) 0px 3px 8px;" src="../../assets/image/user.jpg">
                            <div
            ...
            <textarea v-if="currentChatId" v-model="input" placeholder="请输入"
                class="px-[12px] py-[4px] h-[66px] w-[calc(100%-24px)] text-[14px] leading-[22px] resize-none"
                :disabled="!currentChatId" @keydown.enter.prevent="askGemini" />

            <div v-if="currentChatId" class="mx-[5px] h-[38px]">
                <template v-if="needToken">
                    <a-input-group v-if="isEditToken" class="mt-[5px]">
                        <a-input v-model:model-value="token" class="!w-[340px] mr-[12px]" size="small"
                            placeholder="请输入token" allow-clear />
                        <a-button type="outline" size="small" @click="isEditToken = false;">
                            确定
                        </a-button>
                    </a-input-group>
                    <a-button v-else :status="token ? 'success' : 'danger'" shape="round" size="small"
                        class="float-start mt-[5px]" @click="isEditToken = true;">
                        <template #icon>
                ...
                <a-button v-if="(needToken && token && !isEditToken || !needToken) && ready" size="small"
                    class="float-end mt-[5px]" :disabled="answering || writingAnswer || !currentChatId"
                    @click="askGemini">
                    发&nbsp;&nbsp;送
                </a-button>
                <div class="clear-both" />
        ...
        <a-modal v-model:visible="newChatInputData.show" title-align="start" width="440px" :align-center="false"
            :top="100">
            <template #title>
                新建会话
            </template>
            ...
            <a-input-group class="w-full">
                ...
                <a-input v-model:model-value="newChatInputData.name" allow-clear class="!w-[280px]"
                    placeholder="请输入会话名称" @press-enter="createConversation()" />
            </a-input-group>
        ...
        <a-modal v-model:visible="editChatInputData.show" title-align="start" width="320px" :align-center="false"
            :top="100">
            <template #title>
                会话重命名
            </template>

            <a-input v-model:model-value="editChatInputData.name" allow-clear class="!w-[280px]" placeholder="请输入会话名称"

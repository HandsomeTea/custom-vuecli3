	...
	>
						...
						<div v-if="chat.type === 'user'" class="chat_content mb-[14px]">
							...
							>
								<template v-for="(userChat, s) in chat.content" :key="s">
									...
									<template v-if="userChat.type === 'doc'">
										<br>
										<div
		...
		>
			...
			<a-alert type="warning" class="mb-[10px]">
				...
				输入支持文字和文档；输出仅支持文字。<br>
				分析文档中的图表、图表和表格。<br>
				将信息提取为结构化输出格式。<br>
				回答与文档中的视觉内容和文本内容相关的问题。<br>
				总结文档。<br>
				转写文档内容（例如转写为 HTML），同时保留布局和格式。<br>
			</a-alert>
...
interface AiChatType { type: 'user' | 'model', content: Array<{ type: 'text', data: string, } | { type: 'doc', name: string, data: number }> }
...
const ws = new WebSocket('/ws/ai/doc');
...
}>('ai-doc', ['doc-chat', 'chat-doc-file'], () => Tips.warn('会话太多了，请删除一些会话'));

onMounted(async () => {
	showHistory.value = true;
	const storeChat = await localDB.get('doc-chat');
					...
					return {
						content: [
							...
							...await (async () => {
								...
								const chatFile: Array<{ type: 'doc', data: number, name: string }> = [];
								...
								for (const item of applyFileList.value) {
									...
									const fileId = await localDB.add('chat-doc-file', {
										name: item.file.name,
...
ws.onmessage = async (result: { data: string }) => {
	if (result.data === '&&&end&&&') {
		...
		localDB.updateById('doc-chat', parseInt(currentChatId.value), { chat: currentChatContent.value });
...
const triggerApplyStoragedFile = async (fileId: number) => {
	...
	if (index >= 0) {
		...
		const file = await localDB.getById('chat-doc-file', fileId);
	...
	for (const id of applyStoragedFileList.value) {
		const result = await localDB.getById('chat-doc-file', id);
...
const switchConversation = async (chatId: string) => {
	...
	currentChatContent.value = (await localDB.getById('doc-chat', parseInt(chatId)))?.chat || [];
...
const createConversation = async () => {
	...
	const id = await localDB.add('doc-chat', {
...
const deleteChat = async (chatId: string) => {
	...
	(await localDB.getById('doc-chat', parseInt(chatId)))?.chat.map(a => {
		a.content.map(async s => {
			if (s.type === 'doc') {
				await localDB.removeById('chat-doc-file', s.data as number);
			}
		});
	});

	localDB.removeById('doc-chat', parseInt(chatId));
};
...
const changeChatName = async () => {
	...
	await localDB.updateById('doc-chat', parseInt(editChatInputData.value.id), { name: editChatInputData.value.name });
};
